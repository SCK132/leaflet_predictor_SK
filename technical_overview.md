...
3.  **個別描画** (`plot13VariantPath()`): 各バリアントの経路を色分けして描画（新版UIによる描画処理）。
4.  **統計計算** (`compute13VarStatistics()`): 全完了後、平均着地点・最大偏差を計算。
5.  **凸包描画** (`drawEnsembleHull()`): Turf.jsで着地点群のコンベックスハルを描画。
6.  **海陸判定** (`checkLandSea13Var()`): 各着地点のハイブリッド海陸判定を非同期実行。
...
- **バージョン**: `predictor-cache-v7` / `predictor-tiles-v7`
...
### 設計の要点 (3種の構成案)
- **データ収集の共通化**: 全案共通で「気球 -> LoRa -> 地上局（回収車内端末）」という物理的なダウンリンク経路を想定し、そこから先のクラウド/ローカル処理のバリエーションを設計。
- **案1: Cloud Native型**: 地上局からクラウド (Cloud Run + Firestore) へリレー。開発速度とスケーラビリティ重視。
- **案2: Edge Computing型**: 地上局からクラウド (Cloudflare Workers) へリレー。低遅延と極限のコスト効率重視。
- **案3: Hybrid-Hub型**: 地上局（車載PC）でそのままローカル計算。通信断への最強の耐性を持つ実働重視。
...
- **PWAキャッシュ更新** (`sw.js`):
    - キャッシュバージョンを `predictor-cache-v8` / `predictor-tiles-v8` へ更新。
